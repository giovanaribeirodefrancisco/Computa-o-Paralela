...
void fatorial(int n, mpfr_t* vet) {
	mpfr_init2(vet[0], 5000); // Inicializa com precisão de 64 bits
	mpfr_set_d(vet[0], 1.0, MPFR_RNDU);

	mpfr_init2(vet[1], 5000); // Inicializa com precisão de 64 bits
	mpfr_set_d(vet[1], 1.0, MPFR_RNDU);

	for (int i = 2; i < n; ++i) {
		mpfr_init2(vet[i], 5000); // Inicializa com precisão de 64 bits
...
void soma(int n, mpfr_t* vet, mpfr_t* globalPointer) {
	...
	mpfr_t parcial_local; #pragma omp critical
{
...
double calcular_e_paralelo(int n_termos, int Threads) {
	...
	mpfr_t global;
	mpfr_init2(global, 5000); // Inicializa com precisão de 64 bits
